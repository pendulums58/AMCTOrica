--시계탑의 괴도 이오시프
local s,id=GetID()
function c116000002.initial_effect(c)
	...
	e3:SetDescription(aux.Stringid(116000002,0))
	e3:SetCategory(CATEGORY_COUNTER)
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e3:SetCode(EVENT_LEAVE_FIELD)
	e3:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DELAY+EFFECT_FLAG_CARD_TARGET)
	e3:SetCountLimit(1,116000902)
...
function s.thfilter(c)
	return (c:IsSetCard(0xefb) or c:IsSetCard(0x60a)) and c:IsSpell() and c:IsAbleToHand()
end
