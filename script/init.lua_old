...
-- local mtf=Duel.MoveToField
-- function Duel.MoveToField(...)
	-- local t={...}
	-- if t[4]==LOCATION_PZONE then
		-- t[4]=LOCATION_SZONE
		-- return mtf(table.unpack(t))
	-- else
		-- return mtf(table.unpack(t))
	-- end
-- end
...
function Effect.SetCountLimit(e,max,code,...)
...
function Card.RegisterEffect(c,e,forced,...)
	...
	local mt=_G["c"..code]
	if not mt then mt=_G["c"..c:GetOriginalCodeRule()] end
...
pcall(dofile,"expansions/script/marui.lua")
pcall(dofile,"expansions/script/Green.lua")
pcall(dofile,"expansions/script/Yipi.lua")